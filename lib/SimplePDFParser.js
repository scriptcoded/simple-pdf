...
class SimplePDFParser extends _events["default"] {
  ...
  async parseRaw() {
    const loadingTask = _pdfjsDist["default"].getDocument(new Uint8Array(this.fileBuffer));
  ...
  async extractPageImages(page) {
    ...
    for (let i = 0; i < ops.fnArray.length; i += 1) {
      ...
      switch (fn) {
        case _pdfjsDist["default"].OPS.transform:
          {
            lastMatrix = args;
            break;
          }

        case _pdfjsDist["default"].OPS.paintImageXObject:
          {
